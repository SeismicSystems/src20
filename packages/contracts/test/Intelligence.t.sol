...
contract IntelligenceTest is Test {
    ...
    ) internal pure returns (bytes memory ct) {
    ...
    function testAddRemove(
        ...
        suint256[][] memory allKeys = new suint256[][](4);
        for (uint256 i = 0; i < 4; i++) {
            allKeys[i] = new suint256[](8);
            for (uint256 j = 0; j < 8; j++) {
                while (usedSeeds[seeds[i][j]]) {
                    // Ensures seeds are unique
                    seeds[i][j] = uint256(
                        keccak256(abi.encodePacked(seeds[i][j]))
                    );
                }
                usedSeeds[seeds[i][j]] = true;
        ...
        suint256[] memory savedKeys = new suint256[](4 * 8);
        for (uint256 i = 0; i < 4; i++) {
            for (uint256 j = 0; j < 8; j++) {
        ...
        for (uint256 i = 2; i < intelligence.numKeys(); i++) {
            ...
            for (uint256 j = 0; j < savedKeyIdx; j++) {
                ...
                bool hashMatch = keccak256(abi.encodePacked(savedKeys[j])) ==
                    hashes[i];
