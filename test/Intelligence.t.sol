...
contract IntelligenceTest is Test {
    ...
    function testEncrypt() public {
        bytes memory encryptResult = intelligence.encrypt(
            0,
            sampleMsg
        );
    ...
    function testEncryptAll() public {
        bytes[] memory encryptResult = intelligence.encryptAll(
            sampleMsg
        );
        ...
        assertEq(
            extractCT(encryptResult[0]), 
            directCiphertext[0]
        );
        assertEq(
            extractCT(encryptResult[1]), 
            directCiphertext[1]
        );
    }
    ...
    function testAddRemove(
        ...
        // 
        bytes[] memory encryptAllResult = intelligence.encryptAll(sampleMsg);
